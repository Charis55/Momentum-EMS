...
export default function EventCard({ event, onDeleted = () => { }, isOrganizerView = false }) {
  ...
  function speak() {
    const msg = `${event.name}. Scheduled ${new Date(timing).toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true })} (${timezone}). ${event.description || ""}`;
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "#6b7280" }} tabIndex={0}>
            When: {timing ? new Date(timing).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true }) : "TBA"} ({timezone})
          </p>
          <p style={{ margin: 0, fontSize: 13, color: "#6b7280" }} tabIndex={0}>
